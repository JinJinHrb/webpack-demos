...
/**
 * 
 * @param {string} flag 'onlyDigit', 'captcha', ''
 */
...
/** 
 * undefined 或 null 都返回 true 
 * */
const isNull = a => {
    if(['undefined', 'null'].indexOf(oType(a)) > -1){
        return true;
    }
    return false;
}
...
/** 根据文件的绝对路径获取文件夹路径 */
const getFolderByFilePath = name => {
    const lastSlashIdx = name.lastIndexOf(Path.sep);
    const folderPath = name.slice(0, lastSlashIdx + 1);
    return folderPath;
}
module.exports.getFolderByFilePath = getFolderByFilePath;
...
const _validateDateStringExpArr = [
    ...
    // format: yyyy-mm-dd hh:mi:ss.fff
...
module.exports.regexIndexOf = function(str, regex, startpos) {
    if(oType(str) !== 'string'){
        logger.error('hdlUtil.regexIndexOf #1661 str is not string:', str);
    }
    var indexOf = _L.trim(str).slice(startpos || 0).search(regex);
...
module.exports.regexLastIndexOf = function(str, regex, startpos) {
    if(oType(str) !== 'string'){
        logger.error('hdlUtil.regexLastIndexOf #1673 str is not string:', str);
    }
    regex = (regex.global) ? regex : new RegExp(regex.source, "g" + (regex.ignoreCase ? "i" : "") + (regex.multiLine ? "m" : ""));
    if(typeof (startpos) === "undefined") {
        startpos = _L.trim(str).length;
    } else if(startpos < 0) {
        startpos = 0;
    }
    var stringToWorkWith = _L.trim(str).slice(0, startpos + 1);
    var lastIndexOf = -1;
    var nextStop = 0;
    var result;
...
function parseJsonWithNumber2String(str){
    ...
    try{
        ...
        try{
            ...
            logger.error('hdlUtil #1490 ERROR', e1, '|', str);
        }
    }
...
const removeHtmlComment = html => {
    return html.replace(/<!--[\s\S]*?-->/g, '');
}
module.exports.removeHtmlComment = removeHtmlComment;
...
/**
 * 删除${data}中路径${keyArr}对应的键值对
 * @param {Object} data 
 * @param {String|Array} keyArr 
 */
function delDeepVal(data, keyArr){
    let keyArr1;
    if(oType(keyArr) === 'string'){
        keyArr1 = keyArr.split('.');
    }else if(keyArr instanceof Array){
        keyArr1 = keyArr;
    }else{
        return false;
    }
    ...
    const keyArr0 = keyArr1.slice(0, keyArr1.length - 1);
    if(keyArr0.length < 1){
        const leafKey = keyArr1[0];
        delete data[leafKey];
        return true;
    }

    const result = [];
    getValByIterativeKey(data, keyArr0, result);
    const rtn = result[0];
    if(oType(rtn) !== 'object'){
        return false;
    }

    const leafKey = keyArr1.slice(-1)[0];
    delete rtn[leafKey];
    return true;
}
module.exports.delDeepVal = delDeepVal;

/**
 * 更新${data}中路径${keyArr}对应的键值对
 * @param {Object} data 
 * @param {String|Array} keyArr 
 * @param {*} newValue 
 */
function setDeepVal(data, keyArr, newValue){
    ...
    if(!(keyArr instanceof Array) || keyArr.length < 1){
        logger.error('setDeepVal ERROR - keyArr is empty:', data, keyArr, newValue)
    }
    const keyArr2 = keyArr.length > 1 ? keyArr.slice(0, keyArr.length - 1) : [];
    const lastKey = keyArr.length > 1 ? keyArr.slice(-1)[0] : keyArr[0];
    if( ['undefined', 'null'].indexOf(oType(lastKey)) > -1 ){
        logger.error('setDeepVal ERROR - lastKey is undefined:', data, keyArr, newValue)
    }
    ...
    if(!lastSecondObj || oType(lastSecondObj) !== 'object'){
        logger.error('setDeepVal ERROR #2040 lastSecondObj is not object:', data, keyArr, newValue)
        return;
    }
...
/**
 * 供 limitExecSpeedPromise 用
 */
const _limitExecSpeedListObj = {};
 ...
 */
const limitExecSpeedPromise = function ( { fun, args, taskId, upperLimit = 5 } ) {
    return Q.promise((rsvRoot, rejRoot) => {
        ...
        if(oType(upperLimit) !== 'number'){
            return rejRoot({code: 110, msg: `upperLimit is not a number: ${upperLimit}`});
        }
        if(!_limitExecSpeedListObj[taskId]){
            _limitExecSpeedListObj[taskId] = {
                list: [],
                upperLimit
            }
        }
        const limitExecSpeedList = _limitExecSpeedListObj[taskId]['list'];
...
module.exports.limitExecSpeedPromise = limitExecSpeedPromise;

const consumeLimitExecSpeedList = function(taskId){
    const {list = [], upperLimit = 5} = _limitExecSpeedListObj[taskId];
    let runningNumber = getDeepVal(_limitExecSpeedListObj, [taskId, 'runningNumber']) || 0;
    if(runningNumber < 0){
        logger.error('consumeLimitExecSpeedList #2479 WARNING: runningNumber < 0, runningNumber: ', runningNumber)
        runningNumber = 0;
    }
    if(list.length < 1){
        return;
    ...
    setDeepVal(_limitExecSpeedListObj, [taskId, 'runningNumber'], ++runningNumber);
    if(runningNumber <= upperLimit){
        consumeLimitExecSpeedList(taskId);
    }
    new Promise((rsv) => {
        fun.apply(_this, args).then(feed => {
            rsvRoot(feed);
            rsv(null);
        }, err => {
            rejRoot(err);
        ...
        let runningNumber2 = getDeepVal(_limitExecSpeedListObj, [taskId, 'runningNumber']) || 0;
        setDeepVal(_limitExecSpeedListObj, [taskId, 'runningNumber'], --runningNumber2);
...
const fraction2IntArray = function(arr){
    ...
    return arr.map(a => {
        ...
        if(fracPos < maxFracPos){
            ...
            arr1[0] = parseInt(_L.padEnd(arr1[0], arr1[0].length + diffFracPos, '0'));
            arr1[1] = maxFracPos;
...
module.exports.fraction2IntArray = fraction2IntArray;
...
module.exports.add2str = add2str;
...
module.exports.minus2str = minus2str;
...
module.exports.multiply2str = multiply2str;
...
const divide2str = function(s1, s2, options = {}){
    ...
    if(ceil){
        ...
        if(oType(fixed) === 'number' && fixed < 0){
            ...
            return String( ( q1 + delta /* Math.pow(10, facS1 * -1) */ ).toFixed(facPos) );
...
module.exports.divide2str = divide2str;

const getEnvironment = (key = 'NODE_ENV') => {
    return getDeepVal(process, ['env', key]);
}
